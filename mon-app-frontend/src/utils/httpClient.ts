...
(response) => response,
(error: AxiosError) => {
// Erreur 401 : échec login (identifiants incorrects) ou token expiré
if (error.response?.status === 401) {
  const isLoginRequest = error.config?.url?.includes('/auth/login');
  const serverMessage = (error.response?.data as { message?: string })?.message;
  if (isLoginRequest) {
    return Promise.reject({
      message: serverMessage || 'Identifiants incorrects',
      status: 401,
    });
  }
  ...
  return Promise.reject({
    message: serverMessage || ERROR_MESSAGES.TOKEN_EXPIRED,
    status: 401,
  });
}
